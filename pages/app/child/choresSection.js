...
export default function choresSection() {
  ...
  const ChoreCard = ({ chore, type }) => {
    ...
    const getBackgroundColor = () => {
      switch (type) {
        case "available":
          return "bg-orange-600"; // soft yellow for purchasable chores
        case "bought":
          return "bg-blue-600"; // soft blue for owned chores
        case "completed":
          return "bg-green-600"; // soft green for completed chores
        default:
          return "bg-white";
      }
    };
    ...
    return (
      <div
        className={`border-4 border-black overflow-hidden transition-colors duration-300 bg-white`}
      >
        {/* Brown Header */}
        <div
          className={`${getBackgroundColor()} px-4 py-3 border-b-4 border-black`}
        >
          <div className="flex items-center justify-center">
            <span className="text-white text-xs ITC-medium">
              {chore.category}
            </span>
          </div>
          <h3 className="text-center ITC-demi text-white text-2xl mt-2">
            {chore.title}
          </h3>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="space-y-2 mb-4">
            {type === "available" && (
              <>
                <div className="flex justify-between items-center pb-2 border-b-2 border-black">
                  <span className="text-lg ITC-medium text-black">
                    Purchase Cost
                  </span>
                  <span className="text-lg ITC-demi text-black">
                    {chore.tokens}
                  </span>
                </div>
                <div className="flex pt-2 justify-between items-center">
                  ...
                  <span className="text-lg ITC-demi text-green-600">
            ...
            {type === "bought" && (
              <>
                <div className="flex justify-between items-center pb-2 border-b-2 border-black">
                  <span className="text-lg ITC-medium text-black">Status</span>
                  <span
                    className={`text-sm ITC-medium px-3 py-1 pt-1.5 border-2 border-black ${
            ...
            {type === "completed" && (
              <>
                ...
                <div className="flex justify-between items-center pt-2">
                  <span className="text-lg ITC-medium text-black">Earned</span>
                  <span className="text-lg ITC-demi text-green-600">
                    ${chore.reward.toFixed(2)}
                  </span>
                </div>
          ...
          {type === "available" && (
            <button
              ...
              className={`w-full py-2 ITC-demi pt-3 text-sm border-2 border-black transition-all ${
                userData.tokens >= chore.tokens
                  ? "bg-stone-300 hover:bg-stone-400 text-black"
          ...
          {type === "bought" && (
            <button
              ...
              className="w-full py-2 ITC-demi pt-3 text-sm border-2 border-black bg-green-400 hover:bg-green-500 transition-all text-gray-800"
            >
              MARK AS DONE
            </button>
          )}
          ...
          {type === "completed" && (
            <div className="w-full py-2 pt-3 ITC-demi text-sm border-2 border-black bg-gray-200 text-center text-gray-600">
              ✓ COMPLETED
            </div>
          )}
        </div>
    ...
    <div className="flex h-screen w-screen">
      ...
      <div className="w-1/4 bg-[#BDCCBA] h-full p-6 flex flex-col gap-6 overflow-y-auto">
        ...
        <div className="bg-white p-6 border-4 border-black">
          ...
          <p className="text-3xl ITC-demi text-yellow-600">{userData.tokens}</p>
        </div>
      ...
      <div className="w-3/4 h-full p-6 overflow-y-auto bg-[#E8F0E6]">
        <Button onClick={() => router.push("/app/child/main")} variant="green">
          Back
        </Button>
        <div className="mb-6 mt-4">
          <h1 className="text-4xl ITC-demi text-gray-800 mb-2">YOUR CHORES</h1>
          <p className="text-gray-600">
            Buy chores with tokens, complete them, and earn real money!
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="flex flex-col">
            <p className="text-2xl mb-4 ITC-demi text-gray-800 flex items-center gap-2">
              Available to Buy
            </p>
              ...
              {boughtChores.length === 0 && (
                <div className="bg-white border-4 border-black p-6 text-center">
                  <p className="text-gray-500 font-medium">
                    No chores yet! Buy some with your tokens.
                  </p>
                </div>
              )}
            </div>
